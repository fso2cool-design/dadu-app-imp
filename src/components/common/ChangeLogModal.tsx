  ...
  return (
    ...
    >
      <div className="space-y-5 -mt-2">
        {/* Header Visual */}
        <div className="bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-600 rounded-2xl p-5 text-white shadow-sm relative overflow-hidden">
          ...
          <p className="text-xs text-indigo-100 mt-1 max-w-lg leading-relaxed">
        ...
        <div className="max-h-[380px] overflow-y-auto space-y-4 pr-1 scrollbar-thin">
          {LATEST_CHANGELOG.highlights.map((cat, idx) => (
            <div
              key={idx}
              className="p-3.5 rounded-xl border border-slate-200/90 bg-white hover:border-indigo-200 transition-colors"
            >
              ...
              <ul className="space-y-1.5">
                {cat.items.map((item, iIdx) => (
                  <li key={iIdx} className="text-xs text-slate-600 flex items-start gap-2 leading-relaxed">
        ...
        <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            ...
            className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold flex items-center justify-center gap-1.5 shadow-sm transition-all cursor-pointer"
          >
